const amount = document.getElementById("amount"),
  ...
  converBtn = document.getElementById("convert-icon"),
  calculateBtn = document.querySelector(".convert");

async function getRate() {
  const respons = await fetch("https://api.exchangerate.host/latest?base=USD");
...
function getList(rates) {
  return Object.keys(rates).map((item) => {
    ...
    li.addEventListener("click", (event) => {
      ...
      const parentSpan =
        selectedListItem.parentElement.parentElement.querySelector("span");
...
async function calculate() {
  ...
  calculateBtn.innerHTML = "Dönüştürülüyor...";
  ...
  try {
    ...
    calculateBtn.innerHTML = "Dönüştür";
